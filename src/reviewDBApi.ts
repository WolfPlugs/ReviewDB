...
const { ToastType, ToastPosition } = webpack.getByProps(["createToast", "showToast"]);
...
export async function getReviews(id: string, offset = 0): Promise<Response> {
    let flags = 0;
    if (!reviewDBSettings.get("showWarning")) flags |= WarningFlag;
    ...
    if (!req.ok) {
        showToast(res.message, ToastType.FAILURE);
        return {
            ...res,
            reviews: [
                {
...
async function patchBlock(action: "block" | "unblock", userId: string) {
    ...
    if (!res.ok) {
        showToast(`Failed to ${action} user`, ToastType.FAILURE);
    } else {
        showToast(`Successfully ${action}ed user`, ToastType.SUCCESS);
